 ...
 * should be terminated on first fail. You can define global options for all
 * commands (like Drupal root and uri). The option -y is always set, as it
 * makes sense in a task runner.
 *
 * ``` php
 * $this->taskDrushStack()
...
class DrushStack extends CommandStack {
  ...
  /**
   * @param bool $force force revert even if Features assumes components' state
   *   are default
   * @param string $excludedFeatures space-delimited list of features to
   *   exclude from being reverted
   *
   * @return $this
   */
